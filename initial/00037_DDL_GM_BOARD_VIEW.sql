...
create or replace view gm_board_css as
  ...
  from (
    ...
    union all
    select '[hack=hack]{background-color:black;}' css, 0 display_order from dual
    union all
    --Board CSS
...
create or replace view gm_board_css as
  ...
  from (
    -- start CSS
    select -1 game_id, '<style type="text/css">' css, 0 display_order from dual
    union all
    select -1 game_id, '[fake=fake] { background-color:black; }' , 0 display_order from dual
    union all
    --Other CSS
    select G.game_id, C.css_selector || ' ' || C.css_definition board_css, C.css_order display_order
    from gm_games G
    join gm_boards B on G.game_id = B.game_id
    join gm_gamedef_css C on B.board_type = C.gamedef_code
    union all
    -- end CSS
    select -1 game_id,'</style>' css, 100000 display_order from dual
