<<<<<<< Updated upstream
CREATE OR REPLACE package body APEX_GM.GM_GAME_LIB as
  ...
  begin
    ...
    for z in 1..v_move_choices.count loop
      ...
      for c in 1..length(move_choice) loop
        ...
        if move_step = 'L' or move_step='O' then    
...
create or replace package body GM_GAME_LIB as
  ...
  begin
    ...
    for z in 1..v_move_choices.count loop
      move_choice := v_move_choices(z);
      case  when move_choice = '+' then move_choice  :='^:v:<:>';
            when move_choice = 'x' then move_choice   :='\:/:L:J';
            when move_choice = 'O' then move_choice :='^:v:<:>:\:/:L:J';
      end;
      new_x := null;
      new_y := null;
      dbms_output.put_line('');
